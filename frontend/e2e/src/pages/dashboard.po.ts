...
async getUserNameOnly(): Promise<string> {
  ...
  return fullText.split('|')[0].trim().replace(/\s+/g, ' ');
}
  ...
  async getWeightFromCard(): Promise<string> {
    ...
    await browser.wait(EC.visibilityOf(valueEl), 5000);
  ...
  async hasNoData(): Promise<boolean> {
    const noDataMessage = element(by.cssContainingText('h4.tx-purple.mg-b-20', 'No Data Found!'));
    return await noDataMessage.isPresent();
  }

 async ensureDataOnDashboard(filePath: string): Promise<void> {
  await this.clickDashboardLink();
  await browser.sleep(2000);

  if (await this.hasNoData()) {
    const sourcesPage = new SourcesPage();

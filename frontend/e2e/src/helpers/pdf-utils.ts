...
export async function clickExportToPDF(): Promise<void> {
  ...
  await browser.wait(EC.elementToBeClickable(link), 10000, '"Export to PDF" link was not clickable');
  await link.click();
  ...
  expect(fileStat.size).toBeGreaterThan(0, 'The downloaded PDF file is empty.');

  console.log(`SUCCESS: File "${path.basename(expectedFile)}" was downloaded and verified successfully.`);
}
